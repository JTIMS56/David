...
- The fancy signal categories ("MILD_BULLISH", "STRONG_BEARISH"): all between 46% and 53%. And, crucially, **they kept swapping places every few days.**
...
![The coin does not care how smart your model is](img/fig1-coinflip.png)
*Two models, one month of live forecasts: the sophisticated one and the naive one traded places around 50% all month.*
...
![In-sample is marketing, out-of-sample is truth](img/fig2-overfit.png)
*Every discovered "edge" (blue) collapsed when retested on unseen data (orange). The best one fell from 77% to 15%.*
...
For weeks, the system showed anomalies I kept explaining away: fills 11 to 74 pips from quoted prices (real slippage on major pairs is under 1 pip), positions stopped out 1 to 2 pips from entry when the stop was 20 pips away, "take-profit" exits that somehow lost money, and every hourly cycle log arriving *twice*.
...
Engineering lesson one, learned the hard way: *most of what looks like a model failure is a data-integrity failure.* Decision layer and execution layer must read the same prices at the same time, or your system is fighting itself.

Engineering lesson two: graduated execution (shadow mode that logs everything and trades nothing, then micro, then full) turns catastrophic bugs into $2 lessons. I will never again run a trading system without it.
...
The verdict, on 325 clean out-of-sample forecasts: **49.8% accuracy**. The 59 trades of the final clean week: 44% win rate, average win smaller than average loss, net minus $71 at full practice size.
...
![Empty ocean, real tide: the detector was never broken](img/fig3-sharpe.png)
*The same engine that scored every FX strategy at zero found the equity premium instantly. The zeros were readings, not malfunctions.*
...
![Noise wins the day, signal wins the decade](img/fig4-scale.png)
*A strategy only exists where its signal line climbs above the noise line. FX never gets there. Equities do, if you hold for years.*
